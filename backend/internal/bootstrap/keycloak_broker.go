...
func (registrar keycloakBrokerRegistrar) EnsureKeycloakBroker(ctx context.Context, tenantID string) (string, string, error) {
	...
	created, err := registrar.oauth.CreateOAuthClient(ctx, application.OAuthClientCreateInput{TenantID: tenantID, ApplicationID: platform.ID, EnvironmentID: environment.ID, OperatorID: "system-keycloak", ClientID: "keycloak-broker", ClientName: "Keycloak Broker", ClientType: "confidential", TokenAuthMethod: "client_secret_basic", AccessTokenTTLSeconds: 900, RefreshTokenTTLSeconds: 2592000, GrantTypes: []string{"authorization_code", "refresh_token"}, RequirePKCE: true, Scopes: []string{"openid", "profile"}, RedirectURIs: []string{redirect}})
	if err != nil {
