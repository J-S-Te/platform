...
> **用途**：供后端开发、数据库评审与联调使用。本文只整理仓库中已经提交的 MySQL 迁移与当前后端代码可直接确认的内容；不会把需求设想或未落地能力当成既有事实。  
> **代码基线**：仓库当前工作区，迁移文件 `backend/migrations/000001` 至 `000026`。  
> **生成日期**：2026-07-21。
...
4. **迁移可追溯**：每张表章节均列出首次定义它的迁移文件；第 000025 号迁移对联邦身份提供商表进行了额外加列，单列在“后续结构变更”中列出。
...
| 联邦身份 | `iam_federated_identity_provider`、`iam_federated_identity_binding`、`iam_federated_login_state` |
| 审计与异步任务 | `audit_event*`、`async_job` |
...
| 联邦提供商密钥及登录状态 | `iam_federated_identity_provider.client_secret_ciphertext`、`iam_federated_login_state.state_hash`、`payload_ciphertext` | 客户端密钥、临时登录载荷采用密文或哈希字段。 |
...
来源：`backend/migrations/000025_add_federated_identity_provider_runtime_configuration.sql`。

- 新增列：`client_id` — `VARCHAR(512) CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NULL`
- 新增列：`callback_uri` — `VARCHAR(2048) CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NULL`
- 新增列：`authorization_scopes` — `JSON NULL`
- 新增列：`client_secret_ciphertext` — `BLOB NULL`
- 新增列：`client_secret_updated_at` — `DATETIME(3) NULL`
