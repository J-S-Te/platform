...
func main() {
	...
	if *limit < 1 || *limit > 100000 || *interval < 0 || root == "" || dsn == "" { log.Fatal("valid root, DSN, limit and interval are required") }
	realRoot, err := filepath.EvalSymlinks(root); if err != nil { log.Fatal(err) }
	db, err := gorm.Open(mysql.Open(dsn), &gorm.Config{}); if err != nil { log.Fatal(err) }
	var rows []row
	...
	for _, item := range rows {
		...
		if clean == "." || filepath.IsAbs(clean) || strings.HasPrefix(clean, ".."+string(os.PathSeparator)) { mismatched++; continue }
		path := filepath.Join(realRoot, clean)
		real, realErr := filepath.EvalSymlinks(path)
		if realErr != nil || (real != realRoot && !strings.HasPrefix(real, realRoot+string(os.PathSeparator))) { missing++; continue }
		file, openErr := os.Open(real); if openErr != nil { missing++; continue }
		hash := sha256.New(); size, copyErr := io.Copy(hash, file); closeErr := file.Close()
		if copyErr != nil || closeErr != nil || size < 0 { mismatched++; continue }
		total += uint64(size)
		if uint64(size) != item.SizeBytes || !strings.EqualFold(hex.EncodeToString(hash.Sum(nil)), hex.EncodeToString(item.SHA256)) { mismatched++ }
	...
	if missing > 0 || mismatched > 0 { os.Exit(2) }
}
