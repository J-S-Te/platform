...
测试/演示环境的一键接入依赖 **本地部署 Agent + Docker Socket + 完整工作区**。正式生产目录也提供独立的受控 Agent，但只允许随发布包审核的 `subsystems.d` 精确目标、不可变镜像和固定 Compose 服务；新增目标通过代码评审发布清单，不由浏览器或服务器临时参数扩权。本文仍只介绍把服务器当**本地开发机**的方式：
