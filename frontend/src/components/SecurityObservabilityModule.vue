...
const alertRules = reactive([
  { id: 'alert_001', name: '认证失败阈值', target: '登录安全', condition: '5 分钟内失败 ≥ 5 次', severity: '高', channel: '站内信、邮件', status: '启用' },
  { id: 'alert_002', name: '审计死信队列', target: 'audit_outbox', condition: 'dead_letter_count > 0', severity: '高', channel: '站内信、邮件', status: '启用' },
