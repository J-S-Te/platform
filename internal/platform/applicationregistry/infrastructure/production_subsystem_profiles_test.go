...
func TestProductionSubsystemProfilesLoadReviewedRepositoryTargets(t *testing.T) {
	...
	if !reflect.DeepEqual(capabilities.SupportedApplicationCodes, []string{"contract_management", "customer_and_opportunity", "customer_portal", "data_analysis", "project_management", "settlement"}) {
	...
	for _, target := range []string{"customer_portal", "data_analysis", "project_management", "settlement"} {
...
func platformModuleRoot(t *testing.T) string {
	...
	for {
		if info, statErr := os.Stat(filepath.Join(directory, "go.mod")); statErr == nil && !info.IsDir() {
			root, evalErr := filepath.EvalSymlinks(directory)
			if evalErr != nil {
				t.Fatal(evalErr)
			}
			return root
		...
		if parent == directory {
			t.Fatal("could not locate platform module root")
		}
		directory = parent
	}
}
